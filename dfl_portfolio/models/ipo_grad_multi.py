from __future__ import annotations

from typing import Any, Dict, List, Optional, Sequence, Tuple

import numpy as np
import torch
from qpth.qp import QPFunction


def _coerce_theta_matrix(theta: Optional[np.ndarray], d: int, k: int) -> np.ndarray:
    if theta is None:
        return np.zeros((d, k), dtype=float)
    arr = np.asarray(theta, dtype=float)
    if arr.ndim == 2 and arr.shape == (d, k):
        return arr
    flat = arr.reshape(-1)
    if flat.shape[0] != d * k:
        raise ValueError(f"theta shape mismatch: got {arr.shape}, expected {(d, k)} or flat length {d*k}")
    return flat.reshape(d, k)


def fit_ipo_grad_multi(
    X_feat: Any,
    Y: Any,
    Vhats: Sequence[Any],
    idx: Sequence[int],
    start_index: Optional[int] = None,
    end_index: Optional[int] = None,
    *,
    delta: float = 1.0,
    epochs: int = 500,
    lr: float = 1e-3,
    batch_size: int = 0,
    qp_max_iter: int = 5000,
    qp_tol: float = 1e-6,
    seed: Optional[int] = None,
    theta_init: Optional[Any] = None,
    lambda_anchor: float = 0.0,
    theta_anchor: Optional[Any] = None,
    tee: bool = False,
    debug_kkt: bool = False,
) -> Tuple[Any, Any, Any, Any, List[int], Dict[str, Any]]:
    """IPO-GRAD trainer (multi-feature version) using a differentiable QP layer."""
    X_feat = np.asarray(X_feat, dtype=float)
    Y = np.asarray(Y, dtype=float)
    if X_feat.ndim != 3:
        raise ValueError(f"fit_ipo_grad_multi: X_feat must be (N,d,K), got {X_feat.shape}")
    if Y.ndim != 2:
        raise ValueError(f"fit_ipo_grad_multi: Y must be (N,d), got {Y.shape}")
    N, d, k = X_feat.shape
    if Y.shape != (N, d):
        raise ValueError("fit_ipo_grad_multi: X_feat,Y shape mismatch")

    s = 0 if start_index is None else max(0, int(start_index))
    e = N - 1 if end_index is None else min(N - 1, int(end_index))
    if s > e:
        raise ValueError("fit_ipo_grad_multi: invalid train range")

    pairs: List[Tuple[int, np.ndarray]] = [
        (int(i), np.asarray(V, float)) for i, V in zip(idx, Vhats) if s <= int(i) <= e
    ]
    if not pairs:
        theta_hat = np.zeros((d, k), dtype=float)
        Z = np.empty((0, d), dtype=float)
        MU = np.empty((0,), dtype=float)
        LAM = np.empty((0, d), dtype=float)
        meta: Dict[str, Any] = {"solver": "ipo_grad_multi", "status": "skipped", "message": "no pairs in range"}
        return theta_hat, Z, MU, LAM, [], meta

    used_idx = [i for (i, _) in pairs]
    X_train = X_feat[used_idx]
    Y_train = Y[used_idx]
    V_train = np.stack([v for (_, v) in pairs], axis=0)

    device = torch.device("cpu")
    if seed is not None:
        seed_int = int(seed)
        torch.manual_seed(seed_int)
        np.random.seed(seed_int)
    x_t = torch.from_numpy(X_train).to(device=device, dtype=torch.float32)  # (B,d,K)
    y_t = torch.from_numpy(Y_train).to(device=device, dtype=torch.float32)  # (B,d)
    V_t = torch.from_numpy(V_train).to(device=device, dtype=torch.float32)  # (B,d,d)

    theta_init_mat = _coerce_theta_matrix(theta_init, d, k)
    theta_anchor_mat = _coerce_theta_matrix(theta_anchor, d, k)
    theta_t0 = torch.from_numpy(theta_init_mat).to(device=device, dtype=torch.float32)
    theta_anchor_t = torch.from_numpy(theta_anchor_mat).to(device=device, dtype=torch.float32)

    theta = torch.nn.Parameter(theta_t0.clone())
    opt = torch.optim.Adam([theta], lr=float(lr))

    n_train = int(x_t.shape[0])
    if batch_size <= 0 or batch_size > n_train:
        batch_size_eff = n_train
    else:
        batch_size_eff = int(batch_size)

    alpha = 1.0 - float(delta)
    if not (0.0 < float(delta) < 1.0):
        raise ValueError(f"fit_ipo_grad_multi: delta must be in (0,1); got {delta}")

    eye_d = torch.eye(d, device=device, dtype=torch.float32).unsqueeze(0)
    one = torch.ones(1, d, device=device, dtype=torch.float32)

    patience = 50
    min_delta = 1e-5
    best_loss = float("inf")
    best_theta = theta.detach().clone()
    no_improve = 0
    global_eq_viol = 0.0
    global_ineq_viol = 0.0

    for epoch in range(int(epochs)):
        perm = torch.randperm(n_train, device=device)
        total_loss = 0.0
        n_batches = 0
        max_eq_viol = 0.0
        max_ineq_viol = 0.0

        for start in range(0, n_train, batch_size_eff):
            idx_b = perm[start : start + batch_size_eff]
            xb = x_t[idx_b]  # (B,d,K)
            yb = y_t[idx_b]  # (B,d)
            Vb = V_t[idx_b]  # (B,d,d)

            opt.zero_grad()

            yhat = (xb * theta.unsqueeze(0)).sum(dim=2)  # (B,d)
            Q = float(delta) * Vb
            eps_q = 1e-5
            Q = Q + eps_q * eye_d.expand_as(Q)

            p = -float(alpha) * yhat
            G = -eye_d.expand(Q.size(0), -1, -1)
            h = torch.zeros(Q.size(0), d, device=device, dtype=torch.float32)
            A = one.expand(Q.size(0), -1, -1)
            b_eq = torch.ones(Q.size(0), 1, device=device, dtype=torch.float32)

            z_star = QPFunction(maxIter=int(qp_max_iter), eps=float(qp_tol), verbose=-1)(
                Q, p, G, h, A, b_eq
            )

            ret = (z_star * yb).sum(dim=1)
            risk = torch.einsum("bi,bij,bj->b", z_star, Vb, z_star)
            loss_i = -float(alpha) * ret + 0.5 * float(delta) * risk
            if float(lambda_anchor) > 0.0:
                loss_i = loss_i + float(lambda_anchor) * torch.sum((theta - theta_anchor_t) ** 2)
            loss = loss_i.mean()

            loss.backward()
            opt.step()

            total_loss += float(loss.detach().cpu().item())
            n_batches += 1
            if debug_kkt:
                eq_viol = (z_star.sum(dim=1) - 1.0).abs().max().item()
                ineq_viol = torch.clamp(-z_star, min=0.0).max().item()
                max_eq_viol = max(float(eq_viol), max_eq_viol)
                max_ineq_viol = max(float(ineq_viol), max_ineq_viol)

        if n_batches > 0:
            avg_loss = total_loss / float(n_batches)
            global_eq_viol = max(global_eq_viol, max_eq_viol)
            global_ineq_viol = max(global_ineq_viol, max_ineq_viol)
            if avg_loss < best_loss - min_delta:
                best_loss = avg_loss
                best_theta = theta.detach().clone()
                no_improve = 0
            else:
                no_improve += 1
            if debug_kkt:
                print(
                    f"[IPO-GRAD-MULTI] epoch={epoch+1}/{epochs} loss={avg_loss:.6f} "
                    f"eq_viol={max_eq_viol:.2e} ineq_viol={max_ineq_viol:.2e}"
                )
            elif tee:
                print(f"[IPO-GRAD-MULTI] epoch={epoch+1}/{epochs} loss={avg_loss:.6f}")
            if no_improve >= patience:
                if debug_kkt or tee:
                    print(f"[IPO-GRAD-MULTI] early stopping at epoch {epoch+1} (best_loss={best_loss:.6f})")
                break

    theta.data.copy_(best_theta)
    theta_hat = theta.detach().cpu().numpy().astype(float)
    Z = np.empty((0, d), dtype=float)
    MU = np.empty((0,), dtype=float)
    LAM = np.empty((0, d), dtype=float)

    meta = {
        "solver": "ipo_grad_multi",
        "status": "completed",
        "epochs": int(epochs),
        "loss_best": float(best_loss),
        "lr": float(lr),
        "batch_size": int(batch_size),
        "qp_max_iter": int(qp_max_iter),
        "qp_tol": float(qp_tol),
        "seed": int(seed) if seed is not None else None,
        "eq_viol_max": float(global_eq_viol),
        "ineq_viol_max": float(global_ineq_viol),
        "debug_kkt": bool(debug_kkt),
        "d": int(d),
        "k": int(k),
    }
    return theta_hat, Z, MU, LAM, used_idx, meta


__all__ = ["fit_ipo_grad_multi"]

